...
def solution(subway_times, current_time): # 지하철 도착시간, 현재시간
    current_minute = func_a(current_time)
    INF = 1000000000
    answer = INF
    for s in subway_times:
        subway_minute = func_a(s)
        if subway_minute >= current_minute: # INF 값을 갱신해주는 것 없이 바로 이후에 오는 것 중 최소값 구함
            answer = subway_minute - current_minute
            break # 지하철 시간표는 오름차순으로 되어 있으므로 break를 해줌!
    if answer == INF:
        return -1
    return answer
